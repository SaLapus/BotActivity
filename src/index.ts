...
Bot.on("ready", () => {
  console.log(`ACTIVITY: Logged in as ${Bot.user?.tag}!`);
  setRandActivity();
});
...
function setRandActivity() {
  const Activities = (name: string | undefined): Discord.ActivityOptions => {
    ...
    return acts[getRandomInt(acts.length)] as Discord.ActivityOptions;
  };
...
function getRandomTag() {
  ...
  try {
    ...
    if (membersToMention) console.log([...membersToMention.values()].map((e) => e.user.tag).join(", "));
